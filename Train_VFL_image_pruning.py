...
def train_server(client1_fx, client2_fx, Y_1, t, batch_id, correct, size):
    ...
    if t == epochs-1 and batch_id == len(train_data)-1:
        save_path3 = f'Results/{dataset}/{model}/level{level}/client{number_client}/p{pruning}/server_c{num_cutlayer}.pth'
...
def train_client(dataloader, client_model_1, client_model_2, t):
    ...
    for batch_id, batch in enumerate(dataloader):
        ...
        if t == epochs-1:            
            ...
            n1 = torch.cat([(fx1* pruning_fx1), Y_2], dim=1)
            n1 = n1.cpu().detach().numpy()
            writer_1.writerows(n1)

            if batch_id == len(train_data)-1:
                save_path1 = f'Results/{dataset}/{model}/level{level}/client{number_client}/p{pruning}/client1_c{num_cutlayer}.pth'
                save_path2 = f'Results/{dataset}/{model}/level{level}/client{number_client}/p{pruning}/client2_c{num_cutlayer}.pth'
                ...
                np.save(f'Results/{dataset}/{model}/level{level}/client{number_client}/p{pruning}/X1_c{num_cutlayer}.npy', X1_s)
                np.save(f'Results/{dataset}/{model}/level{level}/client{number_client}/p{pruning}/fx1_c{num_cutlayer}.npy', client1_fx_s)
...
if __name__ == '__main__':
    ...
    # Define record path
    save_path = f'Results/{dataset}/{model}/level{level}/client{number_client}/p{pruning}/'
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    filename = open(f'Results/{dataset}/{model}/level{level}/client{number_client}/p{pruning}/c{num_cutlayer}_{acti}_b{batch_size}.txt', 'w+')
