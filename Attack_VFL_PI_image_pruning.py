...
# Define rocords
save_path = f'Results_PI/{dataset}/{model}/level{level}/client{number_client}/p{pruning}/shadow{num_shadow}/c{num_cutlayer}/client{number_client}/p{pruning}/'
if not os.path.exists(save_path):
    os.makedirs(save_path)

filename = f'Results_PI/{dataset}/{model}/level{level}/client{number_client}/p{pruning}/shadow{num_shadow}/c{num_cutlayer}/client{number_client}/p{pruning}/c{num_cutlayer}_{attack_label}.txt'
...
# Define data
# if dataset == 'utkface':
data_train = pd.read_csv(f'Results/{dataset}/{model}/level{level}/client{number_client}/p{pruning}/VFL_client1_c{num_cutlayer}_shadow.csv', sep=',', header=None)
data_train =np.array(data_train)
train_set=torch.tensor(data_train[:num_shadow, ], dtype=float)
data_test = pd.read_csv(f'Results/{dataset}/{model}/level{level}/client{number_client}/p{pruning}/VFL_client1_c{num_cutlayer}.csv', sep=',', header=None)
