...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='bank_marketing', help="dataset") # [bank_marketing, credit, census]
    parser.add_argument('--acti1', type=str, default='leakyrelu_2', help="acti")
    parser.add_argument('--acti2', type=str, default='leakyrelu_2', help="acti")
...
def train_client(train_iter, client_model_1, client_model_1_fake, t):
    ...
    for batch, (X, Y) in enumerate(train_iter):
      ...
      loss = ((fx1_fake - (fx1 * pruning_fx1)) ** 2).sum()
...
def test_client(test_iter, client_model_1, client_model_1_fake, server_model, t):
    ...
    for batch, (X, Y) in enumerate(test_iter):
        ...
        pruning_fx1_fake = torch.ones_like(fx1_fake)
        _, zis = torch.sort(fx1_fake, descending=False)
        zi = zis[:, :int(pruning*(fx1_fake.shape[-1]))]
        for i in range(len(zi)):
            pruning_fx1_fake[i, zi[i]] =0
        client1_fx_fake = (fx1_fake * pruning_fx1_fake).clone().detach().requires_grad_(True)
...
if __name__ == '__main__':
    ...
    # Define record path
    save_path = f'Results_MS/{dataset}/{acti1}-{acti2}/num_client{number_client}/p{pruning}/s{num_shadow}/'
    if not os.path.exists(save_path):
      os.makedirs(save_path)

    filename = open(f'Results_MS/{dataset}/{acti1}-{acti2}/num_client{number_client}/p{pruning}/s{num_shadow}/c{num_cutlayer}_b{batch_size}_shadow{num_shadow}_acti1_{acti1}acti2{acti2}.txt', 'w+')
    ...
    save_path1 = f'Results_pruning/{dataset}/{acti1}/num_client2/p{pruning}/client1_c{num_cutlayer}_{acti1}_epoch{epochs-1}_b{len(train_iter)-1}.pth'
    save_path2 = f'Results_pruning/{dataset}/{acti1}/num_client2/p{pruning}/client2_c{num_cutlayer}_{acti1}_epoch{epochs-1}_b{len(train_iter)-1}.pth'
    save_path3 = f'Results_pruning/{dataset}/{acti1}/num_client2/p{pruning}/server_c{num_cutlayer}_{acti1}_{batch_size}_epoch{epochs-1}_b{len(train_iter)-1}.pth'
    ...
    save_path = f'Results_MS/{dataset}/{acti1}-{acti2}/num_client{number_client}/p{pruning}/s{num_shadow}/client1_fake_c{num_cutlayer}_shadow{num_shadow}.pth'
