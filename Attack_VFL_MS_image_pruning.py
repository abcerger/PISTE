...
if __name__ == '__main__':
    ...
    # Define record path
    save_path = f'Results_MS/{dataset1}-{dataset2}/{model}/level{level1}-level{level2}/num_shadow{num_shadow}/c{num_cutlayer}/client{number_client}/p{pruning}/'
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    filename = open(f'Results_MS/{dataset1}-{dataset2}/{model}/level{level1}-level{level2}/num_shadow{num_shadow}/c{num_cutlayer}/client{number_client}/p{pruning}/c{num_cutlayer}_{acti}.txt', 'w+')
