...
def parse_args():
    ...
    parser.add_argument('--noise_scale', default=0.1, type=float, help='noise_scale')
...
def train_server(client1_fx, client2_fx, Y, t,  batch,correct):
    ...
    if t == epochs-1 and batch == len(train_iter)-1:
      save_path3 = f'Results_noise/{dataset}/{acti}/num_client{number_client}/n{noise_scale}/server_c{num_cutlayer}_{acti}_{batch_size}_epoch{t}_b{batch}.pth'
...
def train_client(dataloader, client_model_1, client_model_2,  t):
    ...
    for batch, (X, Y) in enumerate(dataloader):
      ...
      if t >=50:
        ...
        n1 = torch.cat([fx1, X1, YR], dim=1)
        n2 = torch.cat([fx2, X2, YR], dim=1)
      ...
      if t == epochs-1:
        ...
        if batch == len(train_iter)-1:
          save_path1 = f'Results_noise/{dataset}/{acti}/num_client{number_client}/n{noise_scale}/client1_c{num_cutlayer}_{acti}_epoch{t}_b{batch}.pth'
          save_path2 = f'Results_noise/{dataset}/{acti}/num_client{number_client}/n{noise_scale}/client2_c{num_cutlayer}_{acti}_epoch{t}_b{batch}.pth'
...
def shadow_client(dataloader, client_model_1, client_model_2, t):
    ...
    for i in range(50):
        for batch, (X, Y) in enumerate(dataloader):
            ...
            client2_fx = (fx2+noise2).clone().detach()

            # record for attack
            YR = Y.reshape(batch_size,1)
            n1 = torch.cat([client1_fx, X1, YR], dim=1)
            n2 = torch.cat([client2_fx, X2, YR], dim=1)
            n1 = n1.cpu().detach().numpy()
            n2 = n2.cpu().detach().numpy()
...
if __name__ == '__main__':
    ...
    # Define record path
    save_path = f'Results_noise/{dataset}/{acti}/num_client{number_client}/n{noise_scale}/'
    if not os.path.exists(save_path):
      os.makedirs(save_path)

    filename = open(f'Results_noise/{dataset}/{acti}/num_client{number_client}/n{noise_scale}/c{num_cutlayer}_b{batch_size}.txt', 'w+')
