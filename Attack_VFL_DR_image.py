...
def parse_args():
    ...
    parser.add_argument('--attack_batch_size', default=8, type=int, help='batch_size')
...
if __name__ == '__main__':
    ...
    # Define record path
    save_path = f'Results_DR/{dataset1}-{dataset2}/{model}/level{level1}-level{level2}/num_shadow{num_shadow}/c{num_cutlayer}/client{number_client}/n{noise_scale}/'
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    filename = open(f'Results_DR/{dataset1}-{dataset2}/{model}/level{level1}-level{level2}/num_shadow{num_shadow}/c{num_cutlayer}/client{number_client}/n{noise_scale}/c{num_cutlayer}.txt', 'w+')
    ...
    if model == 'resnet':
        ...
        client_model_fake_1 = ResNet18(level=level1, hideen1=hideen, num_classes=num_cutlayer).to(device)
        client_model_fake_1 = torch.load(save_path1)
    ...
    x1_true = np.load(f'Results/{dataset1}/{model}/level{level1}/client{number_client}/n{noise_scale}/X1_c{num_cutlayer}.npy')
    fx1_true = np.load(f'Results/{dataset1}/{model}/level{level1}/client{number_client}/n{noise_scale}/fx1_c{num_cutlayer}.npy')
    ...
    for idx_exp in range(num_exp):
        ...
        for iters in range(Iteration*2):
            for i in range(1):
                ...
                yb1_fake = client_model_fake_1(Gout)
                loss1 = ((yb1_fake - fx1_true[:attack_batch_size,]) ** 2).sum()
        ...
        save_G1 = f'Results_DR/{dataset1}-{dataset2}/{model}/level{level1}-level{level2}/num_shadow{num_shadow}/c{num_cutlayer}/client{number_client}/n{noise_scale}/G1_c{num_cutlayer}.pth'
