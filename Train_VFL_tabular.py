...
def train_server(client1_fx, client2_fx, Y, t,  batch,correct):
    ...
    if t == epochs-1 and batch == len(train_iter)-1:
      save_path3 = f'Results/{dataset}/{acti}/num_client{number_client}/n{noise_scale}/server_c{num_cutlayer}_{acti}_{batch_size}_epoch{t}_b{batch}.pth'
...
def train_client(dataloader, client_model_1, client_model_2,  t):
    ...
    for batch, (X, Y) in enumerate(dataloader):
      ...
      if t == epochs-1:
        ...
        if batch == len(train_iter)-1:
          save_path1 = f'Results/{dataset}/{acti}/num_client{number_client}/n{noise_scale}/client1_c{num_cutlayer}_{acti}_epoch{t}_b{batch}.pth'
          save_path2 = f'Results/{dataset}/{acti}/num_client{number_client}/n{noise_scale}/client2_c{num_cutlayer}_{acti}_epoch{t}_b{batch}.pth'
...
if __name__ == '__main__':
    ...
    # Define record path
    save_path = f'Results/{dataset}/{acti}/num_client{number_client}/n{noise_scale}/'
    if not os.path.exists(save_path):
      os.makedirs(save_path)

    filename = open(f'Results/{dataset}/{acti}/num_client{number_client}/n{noise_scale}/c{num_cutlayer}_b{batch_size}.txt', 'w+')
