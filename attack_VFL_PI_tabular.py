...
save_path1 = f'Results_attack/{dataset}/{cutlayer}/{newshadow}'
if not os.path.exists(save_path1):
  os.makedirs(save_path1)

# Define data
if dataset=='bank_marketing':
  traindata = pd.read_csv(f'Results/{dataset}/{acti}/num_client2/n0/{client_c}_{acti}_{dataset}_c{cutlayer}_shadow.csv', sep=',',header=None)
  traindata = np.array(traindata)
  testdata = pd.read_csv(f'Results/{dataset}/{acti}/num_client2/n0/{client_c}_{acti}_{dataset}_c{cutlayer}.csv',sep=',', header=None)
...
filename=f'Results_attack/{dataset}/{cutlayer}/{newshadow}/{dataset}_{acti}_c{cutlayer}_{client_c}_{attack_label}_{attackepoch}_{newshadow}.txt'
f2 = open(filename, 'w')
